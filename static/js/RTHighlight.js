...
*/
// ---------------------------------------------------------------------------------------------------------------- //
				console.log('1: '+this.inar(ln, s.nl));
				console.log('2: '+this.inar(el.children[i].localName, s.nl));


				if(!oneLine ){
					newLines = '<br>';
				}
				if(els.getAttribute('rthline', 1)){
					newLines = '';
				}
				if(els.getAttribute('rthnline', 1))						fnl = '<br>';
				if(!els.getAttribute('rthnline', 1))					fnl = '';
				...
				// tabs before element
				if(els.attributes.rthtabs){
																		tabs			+= this.gtabs(els.getAttribute('rthtabs', 2));
																		console.log(this.gtabs(els.getAttribute('rthtabs', 2)));
																		console.log(els.getAttribute('rthtabs', 2));
																		console.log(els);
				}
				// if element should be in 1 line and ends there

				this.content		+= tabs+fnl+'<span class="RTH-el">&lt;'+el.children[i].localName+adds+'&gt;</span>'+newLines;
				
				// 
				if(!oneLine && els.getAttribute('rthline', 1) == 'false' || !endSlash && els.getAttribute('rthline', 1) == 'false'){
																		this.content	+= newLines+tabs+this.gtabs(1)+text+newLines;// '<br>' changed for newLines
				}
				if(oneLine || els.getAttribute('rthline', 1)){
																		this.content	+= text;
				}
				// loops for inner elements
				this.linesLoop(el.children[i], i);

				// als het element niet met / in de zelfde lijn
				if(endSlash)											this.content 	+= '<br>';

				//console.log(els.getAttribute('rthline', 2));
				// als het element niet op 1 lijn moet zijn
				if(!oneLine)											this.content	+= '<br>';
				this.content += fnl;
				// als het niet op 1 lijn is, als het element niet met / in de zelfde lijn
				if(!oneLine || !this.inar(el.children[i].localName, s.endNodes) && els.getAttribute('rthline', 1)!='true'){
																		this.content 	+= tabs;
				}
				if(!this.inar(el.children[i].localName, s.endNodes))	this.content 	+= '<span class="RTH-el">&lt;/'+el.children[i].localName+'&gt;</span>';
